...
- (void)initView
{
    ...
    overView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:.6];
    [window addSubview:overView];
    
    _baseView = [[UIView alloc] initWithFrame:CGRectMake((kScreen_Width-(512/2))/2, (kScreen_Height-100)/2, 512/2, 100)];
    ...
    remindLab.font = [UIFont boldSystemFontOfSize:16];
...
//    remindLab.text = @"文件正在下载，是否确定取消下载?";
    ...
    for (int i = 0; i < titleArr.count; i++) {
        ...
        if (i == 1) {
            [btn setTitleColor:[UIColor colorWithHexString:@"#FD0400"] forState:UIControlStateNormal];
    ...
    //分割线
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(baseView.width/2.0, 0, .5, 44)];
